...
var params=[remote.getGlobal('sharedObj').customerId];
var isbn;
db._query("select * from  queryOrder where customerId=?",params,(err,res) =>{
    if(err) throw err;
    res.render('order',{result:res});}//渲染result模板，并传递res数据
);
function editTd(id) {
